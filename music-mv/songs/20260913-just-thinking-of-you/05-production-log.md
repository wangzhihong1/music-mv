...
| 20260915 | 安装 ComfyUI MuseTalk | Agent / ComfyUI-MuseTalk-KJ + MusicMV 节点 | 因本机 ComfyUI 是 Python 3.13，不能装 mmcv/mmpose。改用 Kijai 原生节点 + `musicmv_musetalk` 贴回下半脸。公共工作流 `06_视频对口型.json`，排队 `MusicMV_tools/queue-musetalk.mjs`。不改 H3 的 `05` | 权重下载中 | 权重齐后重启 ComfyUI，镜 04 短测 |
