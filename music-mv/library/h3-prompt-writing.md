...
本文件把官方 Ref2VA 格式和社区实测，压成可直接写进 `05_H3参考图生视频_4步_SLA` 的规则。写或改任何 H3 分镜提示词前先读这一份。
...
- 脚本里的单镜时长是剪辑时长，由导演定在 0.5–15 秒，不要为了好生成改成同一种秒数。H3 一次生成的长度可以和剪辑时长不同：短镜头可以生成后再剪到 0.5 秒或 1 秒。
- 脸重要的镜头：约 0.8–1.0MP。需要先跑通时可以从 4–6 秒试，再按该镜的剪辑时长加长或剪短。横版参考 1216×672 或 1280×720。
