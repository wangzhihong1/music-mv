...
const [, , outputPath, title, date, brief = ''] = process.argv

if (!outputPath || !title || !date) {
  console.error('用法：node scripts/create-song-json.mjs <输出路径> <歌名> <日期> [创作描述]')
  process.exit(1)
}
...
const song = {
  ...
  date,
  status: '企划中',
  coreStatement: brief && brief !== '（未提供，可后续补充）' ? brief : '',
  metadata: {
    genre: '',
    mood: '',
    tempo: '',
