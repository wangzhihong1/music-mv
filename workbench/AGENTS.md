...
- 展示从企划到成片的流程状态；点击流程步骤会打开对应内容。主角图片步骤同时展示 `01_人物三视图` 中英文共享提示词；主角图片、分镜视频、修复超分和成片以磁盘上的实际文件为准。
- 在「本机成片」中预览主角图片、场景辅助图、分镜视频、超分中间件、成片和原曲。
- 对照歌词、风格提示词、男/女主角三视图共享提示词和 MV 分镜，支持复制。点击「主角图片」时展示可粘贴到 ComfyUI `01_人物三视图` 的中英文定妆词，并预览磁盘上的主角参考图。
