...
- 本地开发时，`plugins/studio-library-plugin.js` 只从 `../music-mv/` 读取 `song.json`、Markdown 和 `mvs/` 中的生成成品，通过 `/studio-media/` 提供预览，不得把数据或媒体复制进 `src/`。
- 建歌、校验 `song.json`、音频分析属于 `music-mv/`，不要写进本目录。

## 页面职责

- 展示创作中 / 已完成歌曲、本机生成、灵感池和公共素材。
- 展示从企划到成片的流程状态；点击流程步骤会打开对应内容。关键帧、镜头、修复超分和成片以磁盘上的实际文件为准。
- 在「本机成片」中预览定妆图、场景关键帧、原始镜头、超分中间件、成片和原曲。
...
- 修改 UI 后，在浏览器中走一遍：切歌、切分类、总览、MV 对齐、本机成片、点击流程步骤、复制。
